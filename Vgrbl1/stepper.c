...
void st_wake_up() 
{
  ...
  if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) { *ioPort[ENpin] |= ioBit[ENpin]; }
  else { *ioPort[ENpin] &= ~ioBit[ENpin]; }
...
void st_go_idle() 
{
  ...
  //else { STEPPERS_DISABLE_PORT &= ~(1<<STEPPERS_DISABLE_BIT); }
  if (pin_state) { *ioPort[ENpin] |= ioBit[ENpin]; }
  else { *ioPort[ENpin] &= ~ioBit[ENpin]; }
}
...
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
  ...
  if(st.dir_outbits & (0b00100000)) *ioPort[XDpin] |= ioBit[XDpin]; else *ioPort[XDpin] &= ~ioBit[XDpin]; 
  if(st.dir_outbits & (0b01000000)) *ioPort[YDpin] |= ioBit[YDpin]; else *ioPort[YDpin] &= ~ioBit[YDpin]; 
  if(st.dir_outbits & (0b10000000)) *ioPort[ZDpin] |= ioBit[ZDpin]; else *ioPort[ZDpin] &= ~ioBit[ZDpin];
  ...
  //  STEP_PORT = (STEP_PORT & ~STEP_MASK) | st.step_outbits;
	if(st.step_outbits & (0b00000100)) *ioPort[XSpin] |= ioBit[XSpin]; else *ioPort[XSpin] &= ~ioBit[XSpin]; 
	if(st.step_outbits & (0b00001000)) *ioPort[YSpin] |= ioBit[YSpin]; else *ioPort[YSpin] &= ~ioBit[YSpin]; 
	if(st.step_outbits & (0b00010000)) *ioPort[ZSpin] |= ioBit[ZSpin]; else *ioPort[ZSpin] &= ~ioBit[ZSpin]; 
  // #endif  
...
ISR(TIMER0_OVF_vect)
{
  ...
  //*ioPort[XSpin] &= ~ioBit[XSpin];
  //*ioPort[YSpin] &= ~ioBit[YSpin];
  //*ioPort[ZSpin] &= ~ioBit[ZSpin];
  if(step_port_invert_mask & (0b00000100)) *ioPort[XSpin] |= ioBit[XSpin]; else *ioPort[XSpin] &= ~ioBit[XSpin]; 
  if(step_port_invert_mask & (0b00001000)) *ioPort[YSpin] |= ioBit[YSpin]; else *ioPort[YSpin] &= ~ioBit[YSpin]; 
  if(step_port_invert_mask & (0b00010000)) *ioPort[ZSpin] |= ioBit[ZSpin]; else *ioPort[ZSpin] &= ~ioBit[ZSpin]; 
  ...
  { 
	...
	if(st.step_bits & (0b00000100)) *ioPort[XSpin] |= ioBit[XSpin]; else *ioPort[XSpin] &= ~ioBit[XSpin]; 
	if(st.step_bits & (0b00001000)) *ioPort[YSpin] |= ioBit[YSpin]; else *ioPort[YSpin] &= ~ioBit[YSpin]; 
	if(st.step_bits & (0b00010000)) *ioPort[ZSpin] |= ioBit[ZSpin]; else *ioPort[ZSpin] &= ~ioBit[ZSpin];  
  }
#endif
...
void st_reset()
{
  ...
  *ioDdr[XSpin] |= ioBit[XSpin];
  *ioDdr[YSpin] |= ioBit[YSpin];
  *ioDdr[ZSpin] |= ioBit[ZSpin];

  *ioDdr[XDpin] |= ioBit[XDpin];
  *ioDdr[YDpin] |= ioBit[YDpin];
  *ioDdr[ZDpin] |= ioBit[ZDpin];
}
...
void stepper_init()
{
  ...
  *ioDdr[ENpin] |= ioBit[ENpin];
  *ioPort[ENpin]|= ioBit[ENpin];
/*
int ccc;
for(ccc=10;ccc>0;ccc--){

      *ioPort[ENpin]|= ioBit[ENpin];    // Set the bit named in mask
      _delay_ms(100);
 
      *ioPort[ENpin]&=~ioBit[ENpin];   // Clear the bit named in mask
      _delay_ms(100); 
    }
*/
